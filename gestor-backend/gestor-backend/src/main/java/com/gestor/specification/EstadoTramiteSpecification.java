// EstadoTramiteSpecification.java
 ...
 * Construye consultas dinámicas para EstadoTramite
 */
public class EstadoTramiteSpecification {

    /**
     * Construye una Specification dinámica basada en los filtros del DTO
     */
    public static Specification<EstadoTramite> conFiltros(BusquedaAvanzadaDTO filtros) {
        return (root, query, criteriaBuilder) -> {
            List<Predicate> predicates = new ArrayList<>();

            // Join con Cliente (para buscar por nombre y DPI)
            Join<EstadoTramite, Cliente> clienteJoin = root.join("cliente");

            // Join con Tramite (para incluir información del trámite)
            Join<EstadoTramite, Tramite> tramiteJoin = root.join("tramite");
     ...
     */
    public static Specification<EstadoTramite> conNombreCliente(String nombre) {
        return (root, query, criteriaBuilder) -> {
     ...
     */
    public static Specification<EstadoTramite> conDpi(String dpi) {
        return (root, query, criteriaBuilder) -> {
     ...
     */
    public static Specification<EstadoTramite> entreFechas(LocalDate inicio, LocalDate fin) {
        return (root, query, criteriaBuilder) -> {
            List<Predicate> predicates = new ArrayList<>();

            if (inicio != null) {
                predicates.add(
                    criteriaBuilder.greaterThanOrEqualTo(root.get("fechaInicio"), inicio)
                );
            }

            if (fin != null) {
                predicates.add(
                    criteriaBuilder.lessThanOrEqualTo(root.get("fechaInicio"), fin)
                );
            }
     ...
     */
    public static Specification<EstadoTramite> conEstado(String estado) {
        return (root, query, criteriaBuilder) -> {
